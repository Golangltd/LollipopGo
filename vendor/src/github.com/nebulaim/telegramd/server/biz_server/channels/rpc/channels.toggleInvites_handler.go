...
func (s *ChannelsServiceImpl) ChannelsToggleInvites(ctx context.Context, request *mtproto.TLChannelsToggleInvites) (*mtproto.Updates, error) {
	...
	glog.Infof("channels.toggleInvites#49609307 - metadata: %s, request: %s", logger.JsonDebugData(md), logger.JsonDebugData(request))
	...
	channelLogic.ToggleInvites(md.UserId, mtproto.FromBool(request.GetEnabled()))
	...
	glog.Infof("channels.toggleInvites#49609307 - reply: {%s}", logger.JsonDebugData(reply))
	return reply, nil
}
