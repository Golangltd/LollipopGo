...
func FromChannelAdminRights(adminRights *mtproto.TLChannelAdminRights) int32 {
	...
	if adminRights.GetChangeInfo() 		{ rights |= CHANGE_INFO }
	if adminRights.GetPostMessages() 	{ rights |= POST_MESSAGES }
	if adminRights.GetEditMessages() 	{ rights |= EDIT_MESSAGES }
	if adminRights.GetDeleteMessages() 	{ rights |= DELETE_MESSAGES }
	if adminRights.GetBanUsers() 		{ rights |= BAN_USERS }
	if adminRights.GetInviteUsers() 	{ rights |= INVITE_USERS }
	if adminRights.GetInviteLink() 		{ rights |= INVITE_LINK }
	if adminRights.GetPinMessages() 	{ rights |= PIN_MESSAGES }
	if adminRights.GetAddAdmins() 		{ rights |= ADD_ADMINS }
	if adminRights.GetManageCall() 		{ rights |= MANAGE_CALL }
...
func ToChannelAdminRights(rights int32) *mtproto.ChannelAdminRights {
	...
	if (rights & CHANGE_INFO) != 0 { adminRights.SetChangeInfo(true) }
	if (rights & POST_MESSAGES) != 0 { adminRights.SetPostMessages(true) }
	if (rights & EDIT_MESSAGES) != 0 { adminRights.SetEditMessages(true) }
	if (rights & DELETE_MESSAGES) != 0 { adminRights.SetDeleteMessages(true) }
	if (rights & BAN_USERS) != 0 { adminRights.SetBanUsers(true) }
	if (rights & INVITE_USERS) != 0 { adminRights.SetInviteUsers(true) }
	if (rights & INVITE_LINK) != 0 { adminRights.SetInviteLink(true) }
	if (rights & PIN_MESSAGES) != 0 { adminRights.SetPinMessages(true) }
	if (rights & ADD_ADMINS) != 0 { adminRights.SetAddAdmins(true) }
	if (rights & MANAGE_CALL) != 0 { adminRights.SetManageCall(true) }
...
func FromChannelBannedRights(bannedRights *mtproto.TLChannelBannedRights) (int32, int32) {
	...
	if bannedRights.GetViewMessages() 	{ rights |= VIEW_MESSAGES }
	if bannedRights.GetSendMessages() 	{ rights |= SEND_MESSAGES }
	if bannedRights.GetSendMedia() 		{ rights |= SEND_MEDIA }
	if bannedRights.GetSendStickers() 	{ rights |= SEND_STICKERS }
	if bannedRights.GetSendGifs() 		{ rights |= SEND_GIFS }
	if bannedRights.GetSendGames() 		{ rights |= SEND_GAMES }
	if bannedRights.GetSendInline() 	{ rights |= SEND_INLINE }
	if bannedRights.GetEmbedLinks() 	{ rights |= EMBED_LINKS }
...
func ToChannelBannedRights(rights, untilDate int32) *mtproto.ChannelBannedRights {
	...
	if (rights & VIEW_MESSAGES) != 0 { bannedRights.SetViewMessages(true) }
	if (rights & SEND_MESSAGES) != 0 { bannedRights.SetSendMessages(true) }
	if (rights & SEND_MEDIA) != 0 { bannedRights.SetSendMedia(true) }
	if (rights & SEND_STICKERS) != 0 { bannedRights.SetSendStickers(true) }
	if (rights & SEND_GIFS) != 0 { bannedRights.SetSendGifs(true) }
	if (rights & SEND_GAMES) != 0 { bannedRights.SetSendGames(true) }
	if (rights & SEND_INLINE) != 0 { bannedRights.SetSendInline(true) }
	if (rights & EMBED_LINKS) != 0 { bannedRights.SetEmbedLinks(true) }
